...
const getDefaultProgress = (): AppProgress => {
  ...
  CATEGORIES.forEach(c => {
    progress[c.id] = { score: 0, lives: 3, isComplete: false };
  });
  return progress as AppProgress;
};

export default function App() {
  const [progress, setProgress] = useState<AppProgress>(getDefaultProgress());
  ...
  const handleUpdateProgress = (categoryId: CategoryId, newProgress: AppProgress[CategoryId]) => {
    setProgress(prev => {
      const updated = {
        ...prev,
